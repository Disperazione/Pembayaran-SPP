...
class TransaksiExport implements FromQuery, WithHeadings, WithMapping, WithCustomStartCell, WithStyles, WithColumnWidths, ShouldAutoSize, WithTitle
{
    use Exportable;
    private $getdata;
    ...
    {
        if ($transaksi->jumlah_bayar < $transaksi->spp->nominal) {
            $kurang = '- Rp. ' . number_format($transaksi->jumlah_bayar - $transaksi->spp->nominal, 2, ',', '.');
        } else if ($transaksi->jumlah_bayar > $transaksi->spp->nominal) {
            $kurang = '+ Rp. ' . number_format($transaksi->jumlah_bayar - $transaksi->spp->nominal, 2, ',', '.');
        } else {
            $kurang = 'Rp. ' . number_format($transaksi->jumlah_bayar - $transaksi->spp->nominal, 2, ',', '.');
        }
